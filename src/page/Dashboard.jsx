...
function Dashboard() {
    ...
    const fetchData = async () => {
        try {
            ...
            const artistList = await fetch(
                'https://admin.soundwave.io.vn/api/ca-si'
            );
        ...
        return songs.map((song, index) => (
            <div className='w-1/3 pr-10 mb-10' key={song.id}>
                <div className='w-full flex items-center border-b border-slate-700 pb-2 cursor-pointer'>
                    <p className='text-xl text-slate-700 font-medium p-6'>
                        {index + 1}
                    </p>
                    <img
    ...
    return (
        <>
            ...
            <section className='bg-medium pt-20 text-white px-10 h-auto tracking-wide'>
                ...
                <div className='flex items-center '>
                    {['Wean', 'Tăng Duy Tân', 'Wean', 'Wean'].map(
                        (item, index) => (
                            <div
                                className='w-1/6 mr-36 flex flex-col'
                                key={index}
                            >
                                ...
                                <p className='text-lg font-medium ml-24 mt-5 mt-3'>
                                    {item}
                                </p>
                            </div>
                        )

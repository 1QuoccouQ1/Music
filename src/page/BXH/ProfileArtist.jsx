    ...
    const folow = async () => {
      try {
        const response = await fetch(
          API_URL + `/check-ca-si-yeu-thich/${user.id}/${id}`,
          {
            method: "GET",
            headers: {
  ...
  return (
    <>
      <div className="bg-medium w-full h-auto pb-40">
        <section className="w-full   pt-7  text-white px-5">
          <div className="relative w-full h-[600px] z-10">
            ...
            <div className="flex flex-wrap items-center justify-center absolute bottom-7 px-3 lg:left-16 gap-8">
              <img
                className="size-60 rounded-full border-8 border-[#0B1928] lg:translate-y-1/4"
              ...
              <div className="">
                ...
                <div className="flex items-center gap-7">
                  <p className="flex items-center  text-sm font-semibold gap-2">
                    <Headphones size={18} className="text-red-600 " /> 1.584.659
                    người nghe hằng tháng
                  </p>
                  <p className="flex  items-center  text-sm font-semibold gap-2 ">
                    <UserRoundPlus size={18} className="text-red-600 " />{" "}
                    5.940.438 người theo dõi
                  </p>
                </div>
                <div className="flex items-center justify-between md:justify-start  gap-5 mt-5">
                  ...
                  {isFollowing ? (
                    <button
                      ...
                      className="flex items-center border-2 box-border border-red-500 px-4 py-2 rounded-full font-medium gap-1"
                    >
                      {" "}
                      <Check size={20} /> Đang theo dõi
                    </button>
                  ) : (
                    <button
                      onClick={handleFollowing}
                      className="flex items-center border-2 box-border bg-gradient-to-r from-[#FF553E] to-[#FF0065] border-red-500 px-4 py-2 rounded-full font-medium gap-1"
                    >
                      {" "}
                      Theo dõi
                    </button>
          ...
          <div className="flex py-4 w-full justify-center gap-10 rounded-b-xl shadow-medium-xl  bg-slate-900">
            <p
              ...
              className={`font-medium hover:text-red-500 cursor-pointer text-sm duration-300  ${
                isSelect === "1" ? "text-red-500" : " text-white"
              } `}
            >
              Nổi Bật
            </p>
            <p
              ...
              className={`font-medium hover:text-red-500 cursor-pointer text-sm duration-300  ${
                isSelect === "2" ? "text-red-500" : " text-white"
              } `}
            >
              Bài hát
            </p>
            <p
              ...
              className={`font-medium hover:text-red-500 cursor-pointer text-sm duration-300  ${
                isSelect === "3" ? "text-red-500" : " text-white"
              } `}
            >
              Single & EP
            </p>
            <p
              ...
              className={`font-medium hover:text-red-500 cursor-pointer text-sm duration-300  ${
                isSelect === "4" ? "text-red-500" : " text-white"
              } `}
            >
              Album
            </p>
        ...
        {isSelect === "1" && (
          <>
            <section className="w-full text-white px-5 pt-10 flex flex-wrap">
              ...
              <div className="flex w-full lg:w-1/3 ">
                <div className="w-full h-full p-3 relative ">
                  <img
                    ...
                    src={artist.singer_background}
                  />
                  <p className="absolute bottom-4 left-7 text-sm w-5/6">
                    {limitCharacters(artist.singer_biography, 120)}
                    <span
                      onClick={handleModal}
                  ...
                  artistSong.map((song, index) => (
                    <SwiperSlide key={song.id} style={{ width: "auto" }}>
                      <div className="text-center flex flex-col  items-start ">
                        ...
                        <p className="flex text-sm text-slate-600 mt-1">
                  ...
                  {[
                    ...
                    <SwiperSlide key={index} style={{ width: "auto" }}>
                      <div className="w-1/6 flex flex-col w-full" key={index}>
                        ...
                        <p className="text-base font-medium ml-2 mt-1 mt-3 w-56 truncate">
                          Sky Tour (Original Motion .....
                        </p>
                        <p className="text-sm font-medium ml-2 text-slate-500">
                          2022
                        </p>
                      </div>
                    </SwiperSlide>
            ...
            <section className="bg-medium pt-10 text-white px-5 h-auto tracking-wide">
              <div className="flex items-center justify-between">
                <h1 className="text-xl font-medium mb-16">
                  {artist.singer_name} trong loạt hit nổi bật
                </h1>
                <div className="flex items-center text-red-600 hover:text-red-600  cursor-pointer duration-300">
                  <p className="text-sm text-red-600 ">Xem Thêm </p>
                  <svg
                  ...
                  artistSong.map((song, index) => (
                    <SwiperSlide key={song.id} style={{ width: "auto" }}>
                      <div className="text-center flex flex-col  items-start ">
                        <img
                          src={song.song_image}
                          className=" mb-3 rounded-xl w-44 h-44"
                        />
                        <p className="text-sm ml-2">{song.song_name}</p>
                        <p className=" text-sm text-slate-600 mt-1 truncate w-52">
                          {song.description}....{" "}
                        </p>
                      </div>
                    </SwiperSlide>
            ...
            <section className="bg-medium pt-10 text-white px-5 h-auto tracking-wide w-full max-w-full">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center text-red-600 hover:text-red-600  cursor-pointer duration-300">
                  <Link to={`/Artist`} className="text-sm text-red-600 ">
                    Xem Thêm{" "}
                  </Link>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                  ...
                  artists.map((artist, index) => (
                    <SwiperSlide key={index} style={{ width: "auto" }}>
                      <div className="text-center">
                        <Link to={`/ProfileArtist/${artist.id}`}  onClick={()=>window.scrollTo(0, 0)}>
                          <img
                            src={artist.singer_image}
                            className="rounded-full mb-3 w-32 h-32 md:w-52 md:h-52"
                          />
                        ...
                        <button className="flex items-center border-2 box-border bg-gradient-to-r from-[#FF553E] to-[#FF0065] border-red-500 px-3 mt-3 mx-auto py-1 rounded-full font-medium gap-1 text-sm ">
                          {" "}
                          <UserRoundPlus
                            size={15}
        ...
        {isSelect === "2" && (
          <>
            {" "}
            <section className="w-full text-white px-5 pt-5 flex ">
              <div className="flex w-full flex-col gap-2">
                <div className="flex items-center justify-between border-b border-slate-500 pb-2">
                  ...
                  <p className="flex items-center font-medium text-sm  cursor-pointer">
                    Lượt nghe
                  </p>
                  <p className="flex items-center font-medium text-sm  cursor-pointer mr-5">
                    {" "}
                    <Filter size={15} className="mr-2" /> Tùy chọn
                  </p>
                </div>
                {artistSong && artistSong.length > 0 ? (
                  <ProfileArtistSong
        ...
        {isSelect === "3" && (
          <>
            {" "}
            <section className="bg-medium w-full h-auto pb-32  pt-5  text-white px-5 ">
              <div className="">
                {artistSong && artistSong.length > 0 ? (
                  <ProfileArtistSong
                    artistSong={artistSong}
          ...
          <>
            <section className="w-full flex gap-10 items-center text-white px-7 pt-10 ">
              ...
              <div className="flex flex-col gap-14 ">
                ...
                <div className="flex text-sm cursor-pointer hover:opacity-90 items-center gap-2 font-medium py-1 px-3 rounded-lg bg-gradient-to-r from-[#FF553E] to-[#FF0065] w-max">
                  {" "}
                  <Play size={15} /> Phát Nhạc
                </div>
              </div>
            </section>
            ...
            <section className="bg-medium pt-10 text-white px-5 h-auto tracking-wide">
              <div className="flex items-center justify-between">
                <h1 className="text-xl font-medium mb-7">
                  Sơn Tùng M-TP Trong Loạt Hit Nổi Bật
                </h1>
              </div>
              <Swiper
                spaceBetween={20}
              ...
              >
                <SwiperSlide style={{ width: "auto" }}>
                  <div className="text-center flex flex-col  items-start ">
                    ...
                    <p className=" text-sm ml-2 text-slate-600 mt-1 truncate w-52">
                      Hoàng Thùy Linh, Sơn tùng mTP, mono....{" "}
                    </p>
                  </div>
                </SwiperSlide>
                <SwiperSlide style={{ width: "auto" }}>
      ...
      {isModal && (
        <div className=" fixed top-0 bottom-0 right-0 left-0 z-20">
          <div className="bg-black h-[608px] bg-opacity-50 text-white  rounded-xl max-w-4xl w-full flex absolute top-1/2 -translate-x-1/2 -translate-y-1/2 left-1/2">
            <div
              className="w-1/3 pr-4  p-6 relative flex flex-col  "
              style={{

    ...
    return (
        <div className='bg-medium h-auto'>
                ...
                <div className='grid grid-cols-4 gap-4'>
                    {countries.map(country => (
                        ...
                        >
                            {/* Ảnh nền */}
                            <div
                                className={`w-full h-[300px] rounded-lg cursor-pointer opacity-60 group-hover:opacity-100 duration-300`}
                                style={{
            ...
            {categories
                .filter(
                    category =>
                        Array.isArray(songs[category.id]) &&
                        songs[category.id].length > 0
                )
                .map(category => (
                    <section
                        key={category.id}
                    ...
                    >
                        ...
                        <div className='flex items-center'>
                            {songs[category.id].slice(0, 5).map(song => (
                                <div
                                    key={song.id}
                                    className='w-1/5 h-[300px] px-3 rounded-md'
                                >
                                    <div
                                        style={{
